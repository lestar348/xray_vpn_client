...
//    }
    ...
    private fun setup() {
        ...
        if (settingsStorage?.decodeBool(ConfigurationConstants.PREF_PREFER_IPV6) == true) {
        ...
        if (settingsStorage?.decodeBool(ConfigurationConstants.PREF_PER_APP_PROXY) == true) {
            val apps = settingsStorage?.decodeStringSet(ConfigurationConstants.PREF_PER_APP_PROXY_SET)
            val bypassApps = settingsStorage?.decodeBool(ConfigurationConstants.PREF_BYPASS_APPS) ?: false
            apps?.forEach {
                try {
                    if (bypassApps)
