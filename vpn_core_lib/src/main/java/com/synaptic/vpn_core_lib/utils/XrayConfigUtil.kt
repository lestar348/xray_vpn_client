...
import com.synaptic.vpn_core_lib.setup.models.ERoutingMode
import com.synaptic.vpn_core_lib.setup.models.ProtocolType
import com.synaptic.vpn_core_lib.setup.models.XrayConfig
import com.synaptic.vpn_core_lib.setup.models.XrayConfig.Companion.DEFAULT_NETWORK
import com.synaptic.vpn_core_lib.setup.models.XrayConfig.Companion.HTTP
...
object XrayConfigUtil {
    ...
    private fun getV2rayNonCustomConfig(context: Context, outbound: XrayConfig.OutboundBean): Result {
        ...
        if (TextUtils.isEmpty(assets)) {
            Log.e(ANG_PACKAGE, "Can't read default config")
            return result
        }
        ...
        if (settingsStorage?.decodeBool(ConfigurationConstants.PREF_LOCAL_DNS_ENABLED) == true) {
            customLocalDns(v2rayConfig)
        }
        if (settingsStorage?.decodeBool(ConfigurationConstants.PREF_SPEED_ENABLED) != true) {
    ...
    private fun inbounds(v2rayConfig: XrayConfig): Boolean {
        try {
            val socksPort = Utils.parseInt(settingsStorage?.decodeString(ConfigurationConstants.PREF_SOCKS_PORT), ConfigurationConstants.PORT_SOCKS.toInt())
            val httpPort = Utils.parseInt(settingsStorage?.decodeString(ConfigurationConstants.PREF_HTTP_PORT), ConfigurationConstants.PORT_HTTP.toInt())
            ...
            val fakedns = settingsStorage?.decodeBool(ConfigurationConstants.PREF_FAKE_DNS_ENABLED)
                ?: false
            val sniffAllTlsAndHttp = settingsStorage?.decodeBool(ConfigurationConstants.PREF_SNIFFING_ENABLED, true)
                ?: true
...
//            }
    ...
    private fun fakedns(v2rayConfig: XrayConfig) {
        if (settingsStorage?.decodeBool(ConfigurationConstants.PREF_FAKE_DNS_ENABLED) == true) {
    ...
    private fun routing(v2rayConfig: XrayConfig): Boolean {
        try {
            routingUserRule(settingsStorage?.decodeString(ConfigurationConstants.PREF_V2RAY_ROUTING_AGENT)
                ?: "", ConfigurationConstants.TAG_AGENT, v2rayConfig)
            routingUserRule(settingsStorage?.decodeString(ConfigurationConstants.PREF_V2RAY_ROUTING_DIRECT)
                ?: "", ConfigurationConstants.TAG_DIRECT, v2rayConfig)
            routingUserRule(settingsStorage?.decodeString(ConfigurationConstants.PREF_V2RAY_ROUTING_BLOCKED)
                ?: "", ConfigurationConstants.TAG_BLOCKED, v2rayConfig)

            v2rayConfig.routing.domainStrategy = settingsStorage?.decodeString(ConfigurationConstants.PREF_ROUTING_DOMAIN_STRATEGY)
                ?: "IPIfNonMatch"
//            v2rayConfig.routing.domainMatcher = "mph"
            val routingMode = settingsStorage?.decodeString(ConfigurationConstants.PREF_ROUTING_MODE) ?: ERoutingMode.GLOBAL_PROXY.value

            // Hardcode googleapis.cn
            val googleapisRoute = XrayConfig.RoutingBean.RulesBean(
                type = "field",
                outboundTag = ConfigurationConstants.TAG_AGENT,
                domain = arrayListOf("domain:googleapis.cn")
            )

            when (routingMode) {
                ERoutingMode.BYPASS_LAN.value -> {
                    routingGeo("ip", "private", ConfigurationConstants.TAG_DIRECT, v2rayConfig)
                }
                ERoutingMode.BYPASS_MAINLAND.value -> {
                    routingGeo("", "cn", ConfigurationConstants.TAG_DIRECT, v2rayConfig)
                ...
                ERoutingMode.GLOBAL_DIRECT.value -> {
                    val globalDirect = XrayConfig.RoutingBean.RulesBean(
                        type = "field",
                        outboundTag = ConfigurationConstants.TAG_DIRECT,
                        port = "0-65535"
                    )
...
//                                    || it.startsWith("full:"))
    ...
    private fun customLocalDns(v2rayConfig: XrayConfig): Boolean {
        try {
            if (settingsStorage?.decodeBool(ConfigurationConstants.PREF_FAKE_DNS_ENABLED) == true) {
                ...
                val proxyDomain = userRule2Domian(settingsStorage?.decodeString(ConfigurationConstants.PREF_V2RAY_ROUTING_AGENT)
                    ?: "")
                val directDomain = userRule2Domian(settingsStorage?.decodeString(ConfigurationConstants.PREF_V2RAY_ROUTING_DIRECT)
                    ?: "")
            ...
            if (v2rayConfig.inbounds.none { e -> e.protocol == "dokodemo-door" && e.tag == "dns-in" }) {
                ...
                val localDnsPort = Utils.parseInt(settingsStorage?.decodeString(ConfigurationConstants.PREF_LOCAL_DNS_PORT), ConfigurationConstants.PORT_LOCAL_DNS.toInt())
    ...
    private fun dns(v2rayConfig: XrayConfig): Boolean {
        try {
            ...
            val blkDomain = userRule2Domian(settingsStorage?.decodeString(ConfigurationConstants.PREF_V2RAY_ROUTING_BLOCKED)
                ?: "")
            if (blkDomain.size > 0) {
