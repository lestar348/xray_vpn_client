...
data class XrayConfig(
    ...
    data class OutboundBean(val tag: String = "proxy",
        ...
        fun getPassword(): String? {
            ...
            } else if (protocol.equals(ProtocolType.SOCKS.name, true)) {
        ...
        fun getSecurityEncryption(): String? {
            return when {
                protocol.equals(ProtocolType.VMESS.name, true) -> settings?.vnext?.get(0)?.users?.get(0)?.security
                protocol.equals(ProtocolType.VLESS.name, true) -> settings?.vnext?.get(0)?.users?.get(0)?.encryption
                protocol.equals(ProtocolType.SHADOWSOCKS.name, true) -> settings?.servers?.get(0)?.method
                else -> null
            }
        }
