...
import com.synaptic.xcorevpn.extensions.toSpeedString
...
import com.synaptic.xcorevpn.util.XrayConfigUtil
...
object XRayServiceManager {
    ...
    fun startV2Ray(context: Context) {
        ...
        val intent = if (settingsStorage?.decodeString(AppConstants.PREF_MODE) ?: "VPN" == "VPN") {
    ...
    fun startV2rayPoint() {
        ...
        if (!v2rayPoint.isRunning) {
            ...
            try {
                val mFilter = IntentFilter(AppConstants.BROADCAST_ACTION_SERVICE)
            ...
            try {
                v2rayPoint.runLoop(settingsStorage?.decodeBool(AppConstants.PREF_PREFER_IPV6) ?: false)
            } catch (e: Exception) {
                Log.d(ANG_PACKAGE, e.toString())
            }

            if (v2rayPoint.isRunning) {
                MessageUtil.sendMsg2UI(service, AppConstants.MSG_STATE_START_SUCCESS, "")
                showNotification()
            } else {
                MessageUtil.sendMsg2UI(service, AppConstants.MSG_STATE_START_FAILURE, "")
                cancelNotification()
            }
        }
    }
    ...
    fun stopV2rayPoint() {
        ...
        MessageUtil.sendMsg2UI(service, AppConstants.MSG_STATE_STOP_SUCCESS, "")
        cancelNotification()
    ...
    private class ReceiveMessageHandler : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            when (intent?.getIntExtra("key", 0)) {
                AppConstants.MSG_REGISTER_CLIENT -> {
                    //Logger.e("ReceiveMessageHandler", intent?.getIntExtra("key", 0).toString())
                    if (v2rayPoint.isRunning) {
                        MessageUtil.sendMsg2UI(serviceControl.getService(), AppConstants.MSG_STATE_RUNNING, "")
                    } else {
                        MessageUtil.sendMsg2UI(serviceControl.getService(), AppConstants.MSG_STATE_NOT_RUNNING, "")
                    }
                }
                AppConstants.MSG_UNREGISTER_CLIENT -> {
                    // nothing to do
                }
                AppConstants.MSG_STATE_START -> {
                    // nothing to do
                }
                AppConstants.MSG_STATE_STOP -> {
                    serviceControl.stopService()
                }
                AppConstants.MSG_STATE_RESTART -> {
                    startV2rayPoint()
                }
                AppConstants.MSG_MEASURE_DELAY -> {
                    measureV2rayDelay()
                }
            }
    ...
    private fun measureV2rayDelay() {
        GlobalScope.launch(Dispatchers.IO) {
            ...
            MessageUtil.sendMsg2UI(service, AppConstants.MSG_MEASURE_DELAY_SUCCESS, result)
        }
    }
    ...
    private fun showNotification() {
        ...
        stopV2RayIntent.putExtra("key", AppConstants.MSG_STATE_STOP)
    ...
    private fun startSpeedNotification() {
        if (mSubscription == null &&
            ...
            settingsStorage?.decodeBool(AppConstants.PREF_SPEED_ENABLED) == true) {
