...
object Utils {
    ...
    fun getRemoteDnsServers(): List<String> {
        val remoteDns = settingsStorage?.decodeString(AppConstants.PREF_REMOTE_DNS) ?: AppConstants.DNS_AGENT
        val ret = remoteDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(AppConstants.DNS_AGENT)
        }
        return ret
    }

    fun getVpnDnsServers(): List<String> {
        val vpnDns = settingsStorage?.decodeString(AppConstants.PREF_VPN_DNS)
            ?: settingsStorage?.decodeString(AppConstants.PREF_REMOTE_DNS)
            ?: AppConstants.DNS_AGENT
    ...
    fun getDomesticDnsServers(): List<String> {
        val domesticDns = settingsStorage?.decodeString(AppConstants.PREF_DOMESTIC_DNS) ?: AppConstants.DNS_DIRECT
        val ret = domesticDns.split(",").filter { isPureIpAddress(it) || isCoreDNSAddress(it) }
        if (ret.isEmpty()) {
            return listOf(AppConstants.DNS_DIRECT)
        }
        return ret
    }
    ...
    // TODO implement start from toggle
    ...
    fun stopVService(context: Context) {
        ...
        MessageUtil.sendMsg2Service(context, AppConstants.MSG_STATE_STOP, "")
    }
    ...
    fun userAssetPath(context: Context?): String {
        if (context == null)
            return ""
        val extDir = context.getExternalFilesDir(AppConstants.DIR_ASSETS)
            ?: return context.getDir(AppConstants.DIR_ASSETS, 0).absolutePath
    ...
    fun getLocale(context: Context): Locale =
        when (settingsStorage?.decodeString(AppConstants.PREF_LANGUAGE) ?: "auto") {
            "auto" ->  getSysLocale()
            "en" -> Locale("en")
            "zh-rCN" -> Locale("zh", "CN")
            "zh-rTW" -> Locale("zh", "TW")
